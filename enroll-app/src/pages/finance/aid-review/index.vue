    ...
    <scroll-view v-if="item" scroll-y class="body">
      ...
      <SCard title="申请信息" :padding="16">
        <SInfoRow label="助学金类型">
          <picker v-if="canPay" mode="selector" :range="aidTypes" :value="aidTypeIndex" @change="onTypeChange">
            <view class="picker-val">{{ item.type || '国家助学金' }} <text class="picker-arrow">▾</text></view>
          </picker>
          <text v-else>{{ item.type || '国家助学金' }}</text>
        </SInfoRow>
        <SInfoRow label="申请金额">
          <view v-if="canPay" class="amount-edit-row">
...
export default {
  ...
  data() {
    return { REVIEW_STATUS, uid: '', item: null, opinion: '已核对打款金额，确认发放。', rejectReason: '', showConfirm: false, showReject: false, showPreview: false, submitting: false, aidTypes: ['国家助学金', '学校助学金', '社会助学金', '临时困难补助'], editAmount: '' }
  },
  computed: {
    aidTypeIndex() {
  ...
  async onShow() {
    ...
    if (this.item) this.editAmount = String(this.item.amount || '')
  },
  methods: {
    onTypeChange(e) {
