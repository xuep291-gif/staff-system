<template>
  <view class="page">
    <SNavBar title="退宿审核" :showBack="true" fallbackUrl="/pages/teacher/home/index" />
    <StatusTabs tabGroup="teacherDormWithdraw" :tabs="tabs" @change="onTabClick" />
    <scroll-view scroll-y class="body">
      <view class="sc">
          ...
          <view class="card-bd">
            <view class="li">
              ...
              <view class="li-info">
                ...
                <text class="li-desc">当前宿舍：{{ item.currentDorm }}</text>
              </view>
        ...
        <SEmpty v-if="filteredList.length === 0" text="暂无退宿申请" />
      </view>
    </scroll-view>
  </view>
</template>
...
export default {
  ...
  onLoad() {
    this.onBusinessStateChange = ({ collection }) => {
      if (collection === 'dormWithdraws') this.refresh(true)
    }
  ...
  async onShow() {
    ...
    this.refresh(true)
    this.activeTab = getActiveKey('teacherDormWithdraw', 'pending')
  },
  methods: {
    onTabClick(key) {
      console.log('[dorm-withdraw] onTabClick key=', key)
      this.activeTab = key
      setActiveKey('teacherDormWithdraw', key)
    },
    refresh(syncChangedTab = false) {
      this.list = getDormReviewList('dormWithdraws').map(item => {
    ...
    syncActiveTabFromLastChange() {
      const change = getLastBusinessChange('dormWithdraws')
      ...
      setActiveKey('teacherDormWithdraw', DORM_KEY_MAP[index] || 'pending')
    },
    goReview(item) {
      rememberStaffBackTarget('/pages/teacher/dorm-withdraw/index')
      uni.navigateTo({ url: `/pages/teacher/dorm-withdraw-review/index?uid=${item.uid}&apiId=${item.applicationId || item.uid}` })
    }
  }
}
</script>
