<template>
  <view class="page">
    <SNavBar title="退宿审核详情" :showBack="true" />
    <scroll-view scroll-y class="body">
      ...
      <SCard v-if="item" title="退宿信息">
        <SInfoRow label="当前宿舍" :value="item.currentDorm" />
        <SInfoRow label="退宿原因" :value="item.reason" />
...
export default {
  name: 'TeacherDormWithdrawReview',
  ...
  methods: {
    async refresh() {
      const res = this.apiId ? await dormitoryApi.getWithdrawDetail(this.apiId) : null
      this.item = res?.data?.code === 0 ? res.data.data : getDormReviewItem('dormWithdraws', this.uid)
    },
    async submit(type) {
      const status = type === 'approved' ? DORM_REVIEW_STATUS.APPROVED : DORM_REVIEW_STATUS.REJECTED
      const action = type === 'approved' ? dormitoryApi.approveWithdraw : dormitoryApi.rejectWithdraw
      await action(this.apiId, { remark: this.remark || (type === 'approved' ? '同意退宿申请' : '不符合退宿条件') })
      updateReview('dormWithdraws', this.uid, status, {
        action: type === 'approved' ? '退宿审核通过' : '退宿审核驳回',
        operator: '班主任',
        remark: this.remark || (type === 'approved' ? '同意退宿申请' : '不符合退宿条件')
      })
      await this.refresh()
