    ...
    <scroll-view scroll-y class="body" v-if="item">
      ...
      <SCard title="佐证材料" :padding="16">
        <view class="material-row">
          <view class="material-item" v-for="m in materials" :key="m.icon">
      ...
      <!-- Review Form -->
      <SCard title="政务处审批意见" :padding="16">
...
const STORAGE_KEY = 'gov_nondorm_list'
...
function saveItemStatus(uid, status, badgeColor) {
  try {
    const raw = uni.getStorageSync(STORAGE_KEY)
    if (raw) {
...
export default {
  ...
  onLoad(options) {
    ...
    if (uid) {
      this.item = loadItem(uid) || { uid: 'gnd-1', name: '赵刚', id: '2026010008', address: '校园路12号', avatar: '赵' }
    } else {
      this.item = { uid: 'gnd-1', name: '赵刚', id: '2026010008', address: '校园路12号', avatar: '赵' }
    }
    if (this.item) {
      if (this.item.status === '已通过') {
        this.submitDone = true
        this.statusSteps = [
          { label: '学生提交', sub: '2026-05-12', done: true, current: false, popping: false },
          { label: '班主任审核', sub: '已通过', done: true, current: false, popping: false },
          { label: '政务处审批', sub: '已通过', done: true, current: false, popping: false },
          { label: '备案完成', sub: '已通过', done: true, current: false, popping: false }
        ]
  ...
  methods: {
    ...
    onApprove() {
      ...
      if (this.item) saveItemStatus(this.item.uid, '已通过', 'ok')
      this.submitDone = true
      this.animateStep(2)
      setTimeout(() => {
...
.material-item { width: 50%; display: flex; align-items: center; padding: 12rpx 0; }
.material-icon { font-size: 40rpx; margin-right: 12rpx; }
.material-label { font-size: var(--fs-12); color: var(--N700); }
