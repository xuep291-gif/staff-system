...
const seed = {
  students,
  fees: [
    { sid: '2026010001', amount: '5,800', payStatus: 'overdue', urgeCount: 2, urgeTimes: ['2026-05-18 09:10', '2026-05-21 10:25'] },
    { sid: '2026010002', amount: '5,800', payStatus: 'overdue', urgeCount: 1, urgeTimes: ['2026-05-21 09:30'] },
    { sid: '2026010008', amount: '5,800', payStatus: 'overdue', urgeCount: 3, urgeTimes: ['2026-05-17 09:00', '2026-05-19 09:00', '2026-05-21 09:00'] },
    { sid: '2026010015', amount: '5,800', payStatus: 'unpaid', urgeCount: 0, urgeTimes: [] },
    { sid: '2026010022', amount: '5,800', payStatus: 'partial', urgeCount: 1, urgeTimes: ['2026-05-20 14:18'] },
    { sid: '2026010030', amount: '5,800', payStatus: 'paid', urgeCount: 0, urgeTimes: [] },
    { sid: '2026010035', amount: '5,800', payStatus: 'paid', urgeCount: 0, urgeTimes: [] },
    { sid: '2026010039', amount: '2,900', payStatus: 'unpaid', urgeCount: 0, urgeTimes: [] },
    { sid: '2026010042', amount: '0', payStatus: 'channel', urgeCount: 0, urgeTimes: [] }
  ],
  documents: [
  ...
  aids: [
    { uid: 'aid-1', sid: '2026010039', amount: '4,000', type: '特殊困难助学金', status: REVIEW_STATUS.PENDING, date: '05-14', logs: [{ node: '学生申请', time: '2026-05-14 16:22', result: '已提交' }] },
    { uid: 'aid-2', sid: '2026010015', amount: '3,000', type: '普通困难助学金', status: REVIEW_STATUS.PENDING, date: '05-15', logs: [{ node: '学生申请', time: '2026-05-15 10:02', result: '已提交' }] },
    { uid: 'aid-3', sid: '2026010027', name: '吴静雯', amount: '2,000', type: '普通困难助学金', status: REVIEW_STATUS.PENDING, date: '05-16', logs: [{ node: '学生申请', time: '2026-05-16 09:18', result: '已提交' }] },
    { uid: 'aid-4', sid: '2026010033', name: '马晓东', amount: '3,500', type: '国家助学金', status: REVIEW_STATUS.REVIEW_PASS, date: '05-13', logs: [{ node: '政务复审', time: '2026-05-13 11:20', result: '复审通过' }] },
    { uid: 'aid-5', sid: '2026010002', amount: '3,000', type: '国家助学金', status: REVIEW_STATUS.PAYMENT_PENDING, date: '05-12', logs: [{ node: '教师终审', time: '2026-05-12 14:10', result: '终审通过，待财务打款' }] },
    { uid: 'aid-6', sid: '2026010008', amount: '2,500', type: '普通困难助学金', status: REVIEW_STATUS.COMPLETED, date: '05-10', logs: [{ node: '财务打款', time: '2026-05-10 16:30', result: '已完成' }] }
  ],
  loans: [
  ...
  refunds: [
    ...
    { uid: 'rf-6', sid: '2026010027', feeType: '退住宿费', reason: '重复缴费退款', amount: 1200, status: REFUND_STATUS.FAILED, applyTime: '2026-05-17 13:20', failureReason: '收款账户状态异常，请核对后重试', logs: [{ node: '财务打款', time: '2026-05-18 09:16', result: '退款失败', remark: '收款账户状态异常' }] }
  ],
  offlineCollections: [
...
export const REFUND_TAB_GROUPS = [
  ...
  { label: '已完结', statuses: [REFUND_STATUS.REFUNDED, REFUND_STATUS.REJECTED, REFUND_STATUS.FAILED], color: 'ok' }
]
...
export function buildReviewStatusSummary(list, role = 'teacher') {
  ...
  return groups.map(group => {
    const items = records.filter(item => group.statuses.includes(item.status))
    return {
      label: group.label,
      count: items.length,
      color: group.color,
...
export function buildFundingReviewSteps(item = {}) {
  ...
  const steps = [
    ...
    { label: '初审', sub: '待进行', done: false, current: false, popping: false },
...
export function filterRefundByTab(list, tabIndex = 0) {
  ...
  return list.filter(item => matchesStatusGroup(item, group.statuses))
}
...
export function filterPaymentByTab(list, tabIndex = 0) {
  ...
  return list.filter(item => group.statuses.includes(item.payStatus || item.paymentStatus))
}
...
export function getFeeList() {
  ...
  return records.map(fee => {
    ...
    const meta = statusMap[fee.payStatus] || statusMap.unpaid
    ...
    const paidAmount = fee.paidAmount !== undefined
      ...
      : fee.payStatus === 'paid'
        ...
        : fee.payStatus === 'partial'
          ...
          : 0
    const dueAmount = fee.payStatus === 'channel' ? 0 : Math.max(expectedAmount - paidAmount, 0)
    const overdueDays = fee.payStatus === 'overdue' ? (fee.overdueDays || 12) : 0
    return {
      ...fee,
      ...student,
      studentNo: fee.sid,
...
export function updateReview(collection, uid, status, log) {
  ...
  if (item) {
    ...
    if (log?.approvedAmount !== undefined) item.approvedAmount = log.approvedAmount
  }
...
export function getReviewList(collection) {
  ...
  return list.map(item => withStudent(collection === 'documents' ? item : adaptReviewStatus(item), metaMap))
}
...
export function getDormReviewList(collection) {
  ...
  return list.map(item => {
    ...
    const meta = dormReviewStatusMeta[item.status] || dormReviewStatusMeta.pending
    return {
      ...item,
      ...student,
      uid: item.uid,
...
export function getRefundList() {
  return loadState('refunds').map(item => {
    const student = getStudent(item.sid) || {}
    const meta = refundStatusMeta[item.status] || refundStatusMeta.pending
    return {
      ...item,
      ...student,
      id: item.sid,
...
export function retryFailedRefund(uid) {
  const list = loadState('refunds')
  const item = list.find(record => record.uid === uid)
  if (item && item.status === REFUND_STATUS.FAILED) {
    item.status = REFUND_STATUS.APPROVED
    item.failureReason = ''
    item.logs = item.logs || []
    item.logs.unshift({
      node: '财务重新发起',
...
export function getOfflineCollectionList() {
  return loadState('offlineCollections').map(item => {
    ...
    return {
      ...
      badgeColor: item.status === 'confirmed' ? 'ok' : 'wa',
...
export function registerOnsiteCollection(details = {}) {
  const student = getStudent(details.sid)
  const fees = loadState('fees')
  const fee = fees.find(record => record.sid === details.sid)
  if (!student || !fee) return null
  const collections = loadState('offlineCollections')
  const pending = collections.find(record => record.sid === details.sid && record.status === 'pending')
  if (pending) return { ...pending, duplicated: true }
  const submittedAt = nowText()
  const item = {
    id: `oc-${Date.now()}`,
    sid: details.sid,
    method: details.method || '现金',
    location: '迎新现场',
    time: submittedAt,
    amount: Number(details.amount) || 0,
    status: 'pending',
    voucher: details.voucher || '现场收款凭证',
    submitter: details.submittedBy || '迎新工作人员',
    logs: [{ node: '现场收款登记', time: submittedAt, result: '已登记，待财务确认' }]
  }
  collections.unshift(item)
  fee.payStatus = 'paid'
  fee.paidAmount = Number(details.amount) || Number(String(fee.amount).replace(/,/g, ''))
  fee.paidAt = submittedAt
  fee.paymentMethod = item.method
  fee.onsitePendingConfirmation = true
  saveState('offlineCollections', collections)
  saveState('fees', fees)
...
export function confirmOfflineCollection(id, details = {}) {
  ...
  if (item && item.status !== 'confirmed') {
    ...
    item.collectionType = details.collectionType || item.collectionType || item.method || ''
    item.collectionRemark = details.remark || ''
    item.confirmedBy = details.confirmedBy || '陈美玲'
    item.method = item.collectionType
    item.logs = item.logs || []
    item.logs.unshift({ node: '线下收款确认', time: item.confirmTime, result: '已确认', remark: item.collectionRemark })
