<template>
  <view class="page">
    ...
    <STabs :tabs="tabs" v-model="activeTab" storage-key="finance-refund-review" />
    ...
    <SBottomSheet v-model="showSheet" title="退费审核">
      <view v-if="currentItem" class="sheet-info">
        ...
        <SInfoRow v-if="currentItem.failureReason" label="失败详情">
          <text class="reason">{{ currentItem.failureReason }}</text>
        </SInfoRow>
      ...
      <template #footer>
        ...
        <view class="sheet-actions single" v-else-if="canRetry">
          <SButton variant="primary" block @click="onRetryPayment">重新发起退款</SButton>
        </view>
      </template>
    </SBottomSheet>
...
import { buildRefundTabs, filterRefundByTab, getLastBusinessChange, getRefundItem, getRefundList, getRefundTabIndex, retryFailedRefund, updateRefund, REFUND_STATUS } from '@/utils/businessState.js'
...
export default {
  ...
  onLoad() {
    if (!guardStaffFeature('refund')) return
  ...
  methods: {
    ...
    onRetryPayment() {
      if (this.submitting || !this.currentItem || !this.canRetry) return
      retryFailedRefund(this.currentItem.uid)
      this.currentItem = getRefundItem(this.currentItem.uid)
      this.refresh(true)
