  ...
  <view class="page">
    ...
    <scroll-view scroll-y class="body" v-if="item">
      ...
      <SCard title="申请信息" :padding="16">
        ...
        <SInfoRow label="申请原因">
          <text class="reason-text">{{ item.reason || '家庭住址距学校较近，为节省住宿费用，申请校外住宿' }}</text>
        </SInfoRow>
        <SInfoRow label="家长意见">同意学生校外住宿申请</SInfoRow>
        <SInfoRow label="申请时间">{{ item.applyTime || '2026-05-12 10:30' }}</SInfoRow>
      </SCard>
...
export default {
  ...
  methods: {
    ...
    onApprove() {
      ...
      if (this.item) {
        updateReview('nonDorm', this.item.uid, 'approved', { node: '政务处审批', result: '审批通过' })
      }
      this.submitDone = true
      this.animateStep(2)
      setTimeout(() => {
