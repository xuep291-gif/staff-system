<template>
  <view class="page">
    ...
    <SCard title="查询票据">
      <SSearchBar v-model="keyword" placeholder="姓名 / 学号 / 票据号 / 时间" />
    </SCard>
      ...
      >
        ...
        <view class="receipt-body">
          <view class="receipt-row">
            ...
            <text v-if="item.studentNo" class="receipt-no">{{ item.studentNo }}</text>
            <text class="receipt-sep">·</text>
            <text class="receipt-type">{{ item.receiptType }}</text>
          </view>
          <text class="receipt-reason">票据号 {{ item.receiptNo }} · {{ item.submittedAt }}</text>
    ...
    <SCard :title="'已处理（' + doneList.length + '笔）'">
      <view class="receipt-item" v-for="item in doneList" :key="item.id" @click="onItemClick(item)">
        ...
        <view class="receipt-body">
          ...
          <text class="receipt-reason">票据号 {{ item.receiptNo }}</text>
          <SAmount :value="item.amount" size="sm" />
...
export default {
  name: 'FinanceReceipt',
  components: { SNavBar, SCard, SBadge, SAmount, SButton, SBottomSheet, SSearchBar },
  data() {
    return {
      showSheet: false,
      currentItem: {},
  ...
  computed: {
    pendingList() {
      return this.filteredList.filter(item => item.status === 'pending')
    },
    doneList() {
      return this.filteredList.filter(item => item.status !== 'pending')
    },
    filteredList() {
      ...
      return this.list.filter(item => [
        item.name, item.studentNo, item.receiptNo, item.submittedAt, item.reprintTime, item.voidTime
      ].some(value => String(value || '').includes(key)))
    }
  },
  onLoad() {
    guardStaffFeature('receipt')
  },
  onShow() {
    this.refresh()
  },
